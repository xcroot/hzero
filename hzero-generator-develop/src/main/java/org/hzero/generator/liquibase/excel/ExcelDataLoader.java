...
public class ExcelDataLoader {
    ...
    public void processData() throws SQLException {
        ...
        for (TableData tableData : tables) {
            String sql = "SELECT * FROM " + tableData.getName();
                ...
                if (metaData != null ) {
                    for (int i = 1; i <= metaData.getColumnCount(); i++) {
                        for (int j = 0; j < tableData.getColumns().size(); j++) {
                            String columnName = tableData.getColumns().get(j).getName().toLowerCase().replace("*", "");
                            if (StringUtils.equals(columnName, metaData.getColumnName(i).toLowerCase()) && !StringUtils.equals("DATE", tableData.getColumns().get(j).getType())) {
                                tableData.getColumns().get(j).setType(metaData.getColumnTypeName(i).toUpperCase());
                                break;
                            }
                        }
                    }
